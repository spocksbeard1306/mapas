...
_react2['default'].render(_react2['default'].createElement(MapContainer, null), document.getElementById('contenedor'));
//React.render(<WMapa/>, document.getElementById('contenedor2'));
//React.render(<Mapa2/>, document.getElementById('contenedor3'));

},{"../component/mapcontainer.jsx":162,"d3":3,"d3.geo.tile":2,"react":158,"topojson":159}],161:[function(require,module,exports){
"use strict";
...
var Mapa = React.createClass({
    ...
    render: function render() {
        return React.createElement("div", { id: "mapa" });
    },
    componentDidMount: function componentDidMount() {
        ...
        var proj = d3.geo.mercator().scale((1 << inizoom) / 2 / Math.PI).translate([mW / 2, mH / 2]);

        var center = proj([loncen, latcen]);

        this.zoom = d3.behavior.zoom().scale(proj.scale() * 2 * Math.PI).scaleExtent([1 << 23, 1 << 27]).translate([mW - center[0], mH - center[1]]).on("zoom", this.draw);

        // Limpia la proyeccion
        proj.scale(1 / 2 / Math.PI).translate([0, 0]);

        this.path = d3.geo.path().projection(proj);

        var svg = d3.select("#mapa").append("svg").attr("height", mH + "px").attr("width", mW + "px").call(this.zoom);
        var g = svg.append("g");
    ...
    draw: function draw() {
        ...
        var gin = this.g.selectAll("path").data(this.computeCalles(this.props.json)).enter();
        gin.append("path").attr("d", this.path).attr("id", function (d) {
            return d.id;
        });
        gin.append("text").style("font-size", "20px").style("color", "black").append("textPath").attr("xlink:href", function (d) {
            return "#" + d.id;
        }).text(function (d) {
    ...
    computeCalles: function computeCalles(osm) {
        var obnodes = {};
    ...
    computeNodos: function computeNodos(osm) {
...
var _mapaJsx = require('./mapa.jsx');

var _mapaJsx2 = _interopRequireDefault(_mapaJsx);
...
var MapContainer = React.createClass({
    ...
    render: function render() {
        return React.createElement(_mapaJsx2["default"], this.state);
    },
    componentDidMount: function componentDidMount() {
        var self = this;
